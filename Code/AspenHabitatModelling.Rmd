...
```{r, eval=F}
SRM <- st_read(here("Data/Spatial/Ecoregions/us_eco_l3.shp")) %>% filter(US_L3NAME == 'Southern Rockies') %>% st_transform(crs=5071)
...
dem.rast <- lapply(dem$elevation,FUN=rast)[1:11]
dem.rast <- sprc(dem.rast)
dem.merge <- merge(dem.rast, first=T)
writeRaster(dem.merge, here("Data/Spatial/DEM/DEM30-mosaic.tif"), overwrite=T)
...
#### 3 cell neighborhood
```{r, eval=F}
dem.mosaic <- rast(here("Data/Spatial/DEM/DEM30-mosaic.tif"))
