...
if(!file.exists(here("Data","Spatial", "Soils"))){
  ...
  # download mean pH of top 0-5 cm
  for(j in lons){
    for(k in lats){
      temp <- paste0(here("Data","Spatial", "Soils", "lat"), k,"_lon-", j, "_meanPh_0_5.tif")
      download.file(paste0("http://hydrology.cee.duke.edu/POLARIS/PROPERTIES/v1.0/ph/mean/0_5/lat",   k, "_lon-", j, ".tif"), temp)
    }
  }
  
  # Merge mean pH files
  ph.files <- list.files(path=here("Data","Spatial", "Soils"), pattern="meanPh_0_5.tif", full.names=T)
  ph.rast <- lapply(ph.files,FUN=rast)
  ph.rast <- sprc(ph.rast)
  ph.merge <- merge(ph.rast, first=T)
  writeRaster(ph.merge, here("Data","Spatial", "Soils", "meanPh_0_5-SRM.tif"))
  
  # download mean percent clay of top 0-5 cm
  for(j in lons){
    for(k in lats){
      temp <- paste0(here("Data","Spatial", "Soils", "lat"), k,"_lon-", j, "_meanClay_0_5.tif")
      download.file(paste0("http://hydrology.cee.duke.edu/POLARIS/PROPERTIES/v1.0/clay/mean/0_5/lat"  , k, "_lon-", j, ".tif"), temp)
    }
  }
  
  # Merge mean clay files
  clay.files <- list.files(path=here("Data", "Spatial", "Soils"), pattern="meanClay_0_5.tif",   full.names=T)
  clay.rast <- lapply(clay.files,FUN=rast)
  clay.rast <- sprc(clay.rast)
  clay.merge <- merge(clay.rast, first=T)
  writeRaster(clay.merge, here("Data", "Spatial", "Soils", "meanClay_0_5-SRM.tif"))
}
```
...
"us_eco_l3.shp")) %>% filter(US_L3NAME == 'Southern Rockies') %>% st_transform(crs=5071) %>% st_buffer(20000)
  ...
  for(j in 2:length(dem.rast)){dem.rast[[j]] <-  project(dem.rast[[j]], dem.rast[[1]], align=T)}
...
if(!file.exists(here("Data","Spatial", "Climate", "TopoFire"))){
  ...
  download.file("https://topofire.dbs.umt.edu/public_data/topofire_weather/normals_old/aet_1981-2010_mean.tif", temp )
  temp <- here("Data","Spatial", "Climate", "TopoFire", "def_1981-2010_mean.tif")
  download.file("https://topofire.dbs.umt.edu/public_data/topofire_weather/normals_old/def_1981-2010_annual_mean.tif", temp )
}
```
...
SRM <- st_read(here("Data", "Spatial", "Ecoregions", "us_eco_l3.shp")) %>% filter(US_L3NAME == 'Southern Rockies') %>% st_transform(crs(dem)) %>% st_buffer(20000)
dem <- dem %>% crop(SRM)

## Projecting both to UTM 13N
elev <- dem %>% terra::project(proj.proj)
...
select.climate.variables <-c("bFFP", "CMD", "CMI", "DD_0", "DD5", "MAP", "MCMT", "MSP", "MWMT", "PPT_at", "PPT_sp", "RH", "SHM", "TD")
dir.create(here("Data", "Spatial", "Downscaled"), showWarnings = FALSE)

for(j in select.climate.variables){
  if(!file.exists(paste0(here("Data", "Spatial", "Downscaled", "ensemble_8GCMs_ssp370_2011_2040_"), j, "-downscaled.tif"))){
    ptm <- proc.time()
    ...
    writeRaster(ds_layer, paste0(here("Data", "Spatial", "Downscaled", "ensemble_8GCMs_ssp370_2011_2040_"), j, "-downscaled.tif"))
    rm(ds_layer) # remove 
...
for(j in select.climate.variables){
  if(!file.exists(paste0(here("Data", "Spatial", "Downscaled", "ensemble_8GCMs_ssp370_2041_2070_"), j, "-downscaled.tif"))){

    ptm <- proc.time()
    clim <- rast(paste0(here("Data", "Spatial", "Climate", "CLimateNA", "ensemble_8GCMs_ssp370_2041_2070", "ensemble_8GCMs_ssp370_2041_2070_bioclim", "ensemble_8GCMs_ssp370_2041_2070_"), j, ".tif"))
    clim <- clim %>% terra::project(proj.proj)
    clim <- clim %>% crop(SRM)
    ...
    writeRaster(ds_layer, paste0(here("Data", "Spatial", "Downscaled", "ensemble_8GCMs_ssp370_2041_2070_"), j, "-downscaled.tif"))
    rm(ds_layer) # remove 
    gc() # clean up garbage (sometimes this helps)
    print(j) # print j so we know how far along our code is
    time <- proc.time() - ptm
...
for(j in select.climate.variables){
  if(!file.exists(paste0(here("Data", "Spatial", "Downscaled", "ensemble_8GCMs_ssp370_2071_2100_"), j, "-downscaled.tif"))){

    ptm <- proc.time()
    clim <- rast(paste0(here("Data", "Spatial", "Climate", "CLimateNA", "ensemble_8GCMs_ssp370_2071_2100", "ensemble_8GCMs_ssp370_2071_2100_bioclim", "ensemble_8GCMs_ssp370_2071_2100_"), j, ".tif"))
    ...
    writeRaster(ds_layer, paste0(here("Data", "Spatial", "Downscaled", "ensemble_8GCMs_ssp370_2071_2100_"), j, "-downscaled.tif"))
    rm(ds_layer) # remove 
    gc() # clean up garbage (sometimes this helps)
    print(j) # print j so we know how far along our code is
    time <- proc.time() - ptm
