...
preprocessing_recipe <- 
  recipes::recipe(dat.train ~ ., data = training(ames_split)) %>%
...
#### Gradient boosting
```{r}
dat.train.df <- dat.train[,c("aspen.presence", pred.vars)] 
dat.test.df <- dat.test[,c("aspen.presence", pred.vars)] 
dat.train.df$aspen.presence <- dat.train.df$aspen.presence %>% factor()
dat.test.df$aspen.presence <- dat.test.df$aspen.presence %>% factor()
...
bt_cls_fit <- bt_cls_spec %>% fit(aspen.presence ~ ., data = dat.train.df)


bind_cols(
    predict(bt_cls_fit, dat.train.df),
    predict(bt_cls_fit, dat.test.df, type = "prob")
  )

gb.mod1 <- xgboost(data = matrix(dat.train), label = dat.train$aspen.presence, max.depth = 2, eta = 1, nthread = 2, nrounds = 2, objective = "binary:logistic")
