...
Species distribution models of tree species often make use of 
# Methods
...
dat <- aspen.sample.pts %>% na.omit() %>% group_by(aspen.presence) %>%  sample_n(size=1000)
...
dat_split <- rsample::initial_split(
  dat <- dat[,c("aspen.presence", pred.vars)], 
  prop = 0.5, 
...
preprocessing_recipe <- 
  recipes::recipe(aspen.presence ~ ., data = training(dat_split)) %>%
...
xgboost_wf <- 
  ...
  add_formula(aspen.presence ~ .)

xgboost_tuned <- tune::tune_grid(
  object = xgboost_wf,
  resamples = dat_cv_folds,
  grid = xgboost_grid,
...
xgboost_score_train <- 
  train_prediction %>%
  yardstick::metrics(aspen.presence, .pred_class) %>%
...
xgboost_score <- test_prediction %>% yardstick::conf_mat(truth=aspen.presence,estimate=.pred_class) 


# predict on entire dataset
new.dat <- aspen.sample.pts
new.dat$aspen.presence <- factor(new.dat$aspen.presence)
new.data <- bake(preprocessing_recipe,new_data=new.dat)


all_prediction <- xgboost_model_final %>%
  # fit the model on all the training data
  fit(
    formula = aspen.presence ~ ., 
    data    = train_processed
  ...
  # predict the sale prices for the training data
  predict(new_data = new.dat) %>%
  bind_cols(new.dat)%>% yardstick::roc_auc(aspen.presence,.pred_class) 




aspen90.pred <- aspen90.prob01
aspen90.pred[all_prediction$cell] <-  all_prediction$'.pred_class' %>% as.numeric() - 1
writeRaster(aspen90.pred, here("Data", "Spatial", "predicted_1981-2010-xgboos.tif"),overwrite=T)
...
xgboost_model_final.fit <- xgboost_model_final  %>% fit(formula = aspen.presence ~ .,  data    = train_processed)
xgb.importance(xgboost_model_final.fit$fit) %>% head()

explainer_xgb <- 
  explain_tidymodels(
    xgboost_model_final.fit, 
    data = train_processed, 
    y = train_processed$aspen.presence,
    label = "xgboost",
    verbose = FALSE
  )
...
aspen90.pred[all_prediction$cell] <-  all_prediction$'.pred_class'
writeRaster(aspen90.pred, here("Data", "Spatial", "predicted_1981-2010.tif"),overwrite=T)
